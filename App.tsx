...
import {
  generateSpecializationData,
  generateActivitiesList,
  generateCompetenciesForActivities,
...
const App: React.FC = () => {
  ...
  const handleGenerateNames = async () => {
    ...
    try {
      alert(
        "جاري توليد أسماء التخصصات والملخص العام. قد يستغرق الأمر بعض الوقت، يرجى الانتظار..."
      );
      ...
      setApiError(
        "فشل في توليد الأسماء أو الملخص. يرجى التأكد من مفتاح API والمحاولة مرة أخرى."
      );
    } finally {
  ...
  const handleGenerateActivities = async () => {
    ...
    try {
      ...
      setApiError(
        "فشل في توليد الأنشطة. يرجى التأكد من مفتاح API والمحاولة مرة أخرى."
      );
    } finally {
      setIsGenerating(false);
    }
  ...
  const handleGenerateSmartCompetencies = async () => {
    ...
    try {
      ...
      const generatedData = await generateCompetenciesForActivities(
        activityTexts
      );
      ...
      generatedData.forEach((item: any) => {
        ...
        if (
          ...
          item.activityIndex < selectedActivities.length
        ) {
          const targetActivity = selectedActivities[item.activityIndex];
          const mainIndex = updatedActivities.findIndex(
  ...
  const handleNext = () => {
    ...
    if (currentStep === 2) {
      ...
      if (hasInsufficient) {
        setValidationError(
          `يجب التأكد من وجود جدارة واحدة على الأقل لكل نشاط (يمكنك استخدام التوليد الذكي).`
        );
        return;
      }
    }
  ...
  return (
    <div
      className="min-h-screen bg-[#f7f9fb] p-4 md:p-10 font-sans text-right"
      dir="rtl"
    >
